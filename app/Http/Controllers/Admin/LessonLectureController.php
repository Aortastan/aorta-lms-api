...
class LessonLectureController extends Controller
{
    public function show($uuid){
        try{
            $lecture = LessonLecture::select('uuid', 'lesson_uuid', 'parent_lecture_uuid', 'order', 'title', 'body', 'file_path', 'url_path', 'file_size', 'file_duration', 'type')->where(['uuid' => $uuid])->first();

            if($lecture){
                // load variants (child lectures dengan tipe berbeda)
                $variants = LessonLecture::select('uuid', 'parent_lecture_uuid', 'order', 'title', 'body', 'file_path', 'url_path', 'file_size', 'file_duration', 'type')
